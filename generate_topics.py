...
def build_prompt_fmt(fmt, n, existing_titles, existing_places, trending=None, perf=""):
    ...
    return (
        ...
        "- B-ROLL PEOPLE BAN: queries (query/query2/archive_query) must NEVER request people, "
"faces or portraits (no man/woman/boy/girl/child/person/victim). Real cases + random "
"stock faces = viewers call it fake and disrespectful. Use places, objects, documents, "
...
def main():
    ...
    _pl = places[-60:] if len(places) > 60 else places
    _first = True
    for _fmt, _cnt in plan.items():
        try:
            if not _first: time.sleep(30)   # TPM bucket sa doplna po minute
            _first = False
            got = extract_json(call_model(build_prompt_fmt(_fmt, _cnt, _tl, _pl, trending, perf)))
            items += got
